...
```{r choose_sample}
library(XML)
data_qc <- readHTMLTable("/path/to/QC/html/miRNAQC.html",which = 1)
...
# Calculate the boundary points of five equal divisions
breaks <- quantile(df$depth, probs = seq(0, 1, by = 0.2))
breaks_rounded <- round(breaks)

# Calculate the five equal intervals for each group and generate interval numbers
...
# Manually set labels and use the rounded boundary points
labels <- paste0("(", breaks_rounded[-length(breaks_rounded)], ",", breaks_rounded[-1], "]")

# Calculate the five equal intervals for each group and generate the interval ranges
df$depth_range <- cut(
  df$depth, 
  breaks = breaks, 
  include.lowest = TRUE, 
...
# To ensure that each group has 5 intervals, check for any empty intervals
...
library(ggsci)
library(ggstar)
selected_samples_df <- read.table("/path/to/xls/file/select_sample.xls")
data <- read.csv("/path/to/table/isomiRs_readscount_isomirdb.csv",header = T)
dt <- melt(data)
names(dt)[1] <- "isomiRs"
...
saveRDS(result,"/path/to/figures/miRNA_readscound_stats_result.rds")
...
ggsave("/path/to/figure/miRNA_readscount_plot_CRC_Normal_sample_select.pdf",width = 10,height = 7)
```

###  Gradient isomiR feature statistics chart
...
judge_column <- function(id) {
  ...
  # Determine which type the isomiR belongs to
...
data <- read.table("/path/to/table/merge_isomiR_seq.tsv",header = T)
dt <- melt(data)
dt$deep <- gsub(".*\\.","",dt$variable)
...
data_m <- read.delim("/path/to/table/hsa.mature.fa",header = F)
...
data <- read.csv("/path/to/table/CPM_select.csv",row.names = 1)
data$isoType <- rownames(data) %>% lapply(judge_column) %>% unlist()
data <- data[data$isoType=="judge_Canonical",]
data <- data[,-609]
write.csv(data,"/path/to/table/isomiRs_CPM_select_mature_608.csv",row.names = T,quote = F)


isoType <- read.csv("/path/to/output/seq.ouf",sep = "\t")
...
write.csv(df_unique,"path/to/table/isomiRs_CPM_select_mature_608.csv",row.names = F,quote = F)
...
```{r vplot}
de_cpm <- "/path/to/table/de_genes_cpm_combat.rds"
de_cpm <- readRDS(url(URLencode(de_cpm), "rb"))
de_cpf <- "/path/to/table/de_genes_irp_combat.rds"
de_cpf <- readRDS(url(URLencode(de_cpf), "rb"))
sampleNames <- "/path/to/table/training_samples.rds"
sampleNames <- readRDS(url(URLencode(sampleNames), "rb"))
data <- read.csv("/path/to/table/CPM_IRP_select_combat.csv",row.names = 1)
###Volcano Plot
stats_fc_i <- de_cpf
...
Type <- read.csv("/path/to/table/sample_data_608.csv")
names(dt)[2] <- "sample"
...
### Comparison of differential isoforms with TCGA
...
raw_ruvg <- "/path/to/table/raw_ruvg_edger_result.rds"
raw_ruvg <- readRDS(url(URLencode(raw_ruvg), "rb"))
raw_ruvg$level <- get_diff_level(raw_ruvg)
de_TCGA <- "/path/to/table/TCGA_CRC_expression_label_select_0.1_5_genes_result.rds"
de_TCGA <- readRDS(url(URLencode(de_TCGA), "rb"))

de_family <- "/path/to/table/de_genes_family_combat.rds"
...
### Differential Heatmap and PCA Plot

```{r heatmap_group}
group <- readxl::read_xlsx("/path/to/table/CRC.xlsx",sheet = "汇总")
...
data <- read.csv("/path/to/table/CPM_select_combat_833.csv",header = T,check.names = F,row.names = 1)
data <- read.csv("/path/to/table/isomiRs_CPM_select_mature_608.csv",header = T,check.names = F,row.names = 1)
...
saveRDS(result,"/path/to/table/de_isomiRs_CPM_select_mature_608.rds")
write.csv(result,"/path/to/table/de_isomiRs_CPM_select_mature_608.csv",row.names = F,quote = F)
###iso各分期差异
data <- "/path/to/table/exp_matrix_list.rds"
...
### pheatmap
iso <- "/path/to/table/raw_ruvg_edger_result.rds"
iso <- readRDS(url(URLencode(iso), "rb"))
mature <- "/path/to/table/mature_raw_ruvg_edger_result.rds"
mature <- readRDS(url(URLencode(mature), "rb"))
isogroup <- "/path/to/table/isogroup_raw_ruvg_edger_result.rds"
isogroup <- readRDS(url(URLencode(isogroup), "rb"))

data <- "/path/to/table/exp_matrix_list.rds"
data <- readRDS(url(URLencode(data), "rb"))

sample_info <- "/path/to/table/sample_info.rds"
...
### Model-based Selected Heatmap
for (type in c("cpm","cpf")) {
    if(type=="cpm"){
      path <- "/path/to/folder/608_rf_miRNA_cpm_diff_beam_30_0.49_T_0_0/"
      number <- 664
    }else if(type=="cpf"){
      path <- "/path/to/folder/608_rf_miRNA_cpf_diff_beam_30_0.49_T_0_0/"
      number <- 710
    }
...
### Isoforms have differences, but isogroups do not have differences.
...
### Draw the boxplot of the proportion of zero values by family
...
### KEGG and other annotations for differential markers
...
group <- ann_col
group$Sample <- rownames(group)
## Comparative Curve Plot for Multiple Models
########################################################################################
...
p <- plot_grid(
  ...
  ncol = 1, 
  rel_heights = c(0.2, 1))
ggsave("/path/to/output/raw_ruvg_edger_model_test_cutoff_sen_sep_line_plot.pdf",p,width = 9,height = 7.5)
##############################################################################################################################
...
TCGA_data <- read.csv("/path/to/table/TCGA_CRC_expression_label_select_0.1_5.csv",check.names = F,row.names = 1)
TCGA_diff <- "/path/to/table/TCGA_CRC_expression_label_select_0.1_5_genes_result.rds"
...
set.seed(1234)
get_diff_level <- function(iso) {return(factor(ifelse(iso[["PValue"]] < 0.05 & abs(iso[["logFC"]]) > 1, ifelse( iso[["logFC"]] > 1 ,'Up','Down'),'No'),levels=c('Up','Down','No')))}
group <- readxl::read_xlsx("/path/to/table/CRC_final.xlsx")
names(group)[1] <- "sample"
ann_col <- data.frame(row.names = group$sample,Type=group$Type,Stage=group$建模分组,Age=group$年龄,Gender=group$性别)
model_result <- "/path/to/table/train_eval_result_v2.rds"
model_result <- readRDS(url(URLencode(model_result), "rb"))
...
for(i in 9:dim(result)[2]){ #Start looping from the third column, because the first and second columns are not genes; they are surstat and surtime
  tdcox <- coxph(Surv(os_time, status) ~ result[,i], data = result)#Start looping through Cox analysis one by one
  tdcoxSummary = summary(tdcox) 
  pvalue=tdcoxSummary$coefficients[,"Pr(>|z|)"] #get p value
  if(pvalue<pFilter){ # set cutoff pvalue<0.05 
    sigGenes=c(sigGenes,i)
    outResult=rbind(outResult,#The merging of the loop results. The loop must have a starting point.
                    cbind(id=i,
                          HR=tdcoxSummary$conf.int[,"exp(coef)"],#Extract the HR (Hazard Ratio) of a single gene
...
# Obtain the names of non-zero coefficients (selected features)
...
theme2 <- theme(axis.ticks.x = element_blank(), 
...
plot(tROC,time=1*365,col="red",title=F,lwd=2) #1-Year ROC
plot(tROC,time=3*365,col="green",add=T,title=F,lwd=2) #3-Year ROC
plot(tROC,time=5*365,col="blue",add=T,title=F,lwd=2) #5-Year ROC
legend("bottomright", 
...
try({cal1 <- calibrate(f2, 
                  cmethod='KM', 
                  method="boot", 
                  u=365, # u needs to be consistent with the time.inc defined in the previous model, which is either 365 or 730
                  m=110, #The sample size for each sampling
                  B=1000) #Number of samplings
}, silent = FALSE)
## m should be determined based on the sample size, as the standard curve generally divides all samples into three groups (displayed as three points in the graph)
...
plot(cal1,lwd=2,lty=1,
     conf.int=T,
     errbar.col=c(rgb(5,110,188 , maxColorValue = 255) ),
...
plot(cal2,lwd=2,lty=1,
     #conf.int=T,
     errbar.col=c(rgb(234,196,19 , maxColorValue = 255) ),
     xlim=c(0,1),ylim=c(0,1),
     add = TRUE,
     xlab="Nomogram predicted survival probability",
     ylab="Observed fraction survival probability",
...
plot(cal3,lwd=2,lty=1,
     conf.int=T,
     errbar.col=c(rgb(137,137,137 , maxColorValue = 255)),
     xlim=c(0,1),ylim=c(0,1),
     add = TRUE,
     xlab="Nomogram predicted survival probability",
     ylab="Observed fraction survival probability",
...
data <- predict_score
names(data)[1] <- "sample"
group <- readxl::read_xlsx("/path/to/table/CRC_final2.xlsx")
names(group)[1] <- "sample"
...
data <- predict_score
names(data)[1] <- "sample"
group <- readxl::read_xlsx("/path/to/table/CRC_final3.xlsx")
names(group)[1] <- "sample"
